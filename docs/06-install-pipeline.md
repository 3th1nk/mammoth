...
- 应答文件名归驱动(rocky9:ks.cfg;ubuntu22:user-data;debian12:preseed.cfg +
  脚本),编排层不硬编码;
- 可选能力 `FamilyDriver{Family() string}`:支持矩阵导出安装家族
  (`GET /api/v1` 的 `distros[].family`),供集成方判断该发行版走哪条安装
  路径。

**声明化评估结论(2026-09-18 决策记录)**:曾落地 Cobbler 式
`distros.json` 声明文件(12 发行版的家族/布局/池能力/代差参数全量
数据化),评估后同日收敛回类型化 Go 注册表。理由:发行版接入的真实
成本在**发现新怪癖 → 改家族模板 Go → 真机验证**(debian13 十一项、
ubuntu 十八项连环修为证),声明文件只覆盖注册面、占比不足 5%,反而
把行为开关移出了带类型、与实现同址、携带事故注释的 switch;且声明
文件会给出"加条目 = 支持发行版"的假象。保留的声明化收益:family
元数据导出、agent 家族的包集参数(真正的"装什么"数据,留在驱动
内)、ubuntu 架构修正。Cobbler 式数据文件若未来有真实运行时数据需求
(社区在部署侧贡献发行版且不经 Go 审查)再评估——与 compat 矩阵的
`MAMMOTH_COMPAT_DIR` 运行时覆盖不同,后者收集的是现场安全追加的行为
事实,而发行版条目编码安装器驱动语义。
