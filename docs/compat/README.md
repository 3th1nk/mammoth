...
| 虚拟软驱随 CD 一起上报 → 挂 ISO 装机时系统盘盘符漂移(sda 被虚拟 CD 抢占;USB 接口不够所致) | Huawei iBMC(官方 FAQ 文档级:解法 = 关闭软驱,见 [huawei.md](huawei.md) 文档推演节) | ⚠️ 预警:现有盘解析按 size+serial(不信任盘符)天然免疫;根治开关 = PATCH VirtualMedia/CD `Oem.Huawei.FloppyDriveEnabled=false`(+If-Match,不支持时 400 须静默跳过);下一轮 2288H V5 真机窗口即可零风险预验 |
| BIOS Setup 的 PXE/网络引导开关无带外 API → PXE 一次性引导被固件静默跳过,部署前提需人进 BIOS | Huawei iBMC(官方 IPMI 文档级候选:Boot Options OEM 参数 62h 有 "boot from PXE enable" 位) | ✅ 已实测证伪(2288H V5/6.41,2026-09-23):62h 读回 bit[1]=0 而 PXE override 实际可用(该位不管 UEFI 网络引导);Set 返回成功但静默丢弃(锁存舞步亦然)——带外改写不存在,部署前提仍需人进 BIOS(见 [huawei.md](huawei.md) IPMI 推演节 §1) |
