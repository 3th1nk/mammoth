...
| `/netboot/*` 机器面(M7,公开) | 与 `/render/*` 同型:**路径/参数中的不可猜测 token 即凭证**。MAC 是**弱标识仅用于路由**(决定该机器拿哪份引导脚本),不是鉴权——知道 MAC 至多能让 iPXE 拿到 exit 回退脚本,拿不到任何凭证;引导树文件按条目 allowlist 平面名服务,无路径穿越面;脚本内容(内核参数含 NFS 地址、ks URL)与安装介质同敏感级——暴露前提是攻击者已在装机 L2 内,该前提下虚拟介质通路同样可被观测 |
